...
export function toLazyValue(lazy: undefined | DefineDataLoaderOptionsBase_LaxData['lazy'], to: RouteLocationNormalizedLoaded, from?: RouteLocationNormalizedLoaded) {
  return typeof lazy === 'function' ? lazy(to, from) : lazy
}
...
export interface UseDataLoaderResult<TData = unknown, TError = ErrorDefault> {
   ...
   * inconsistencies.
   */
  reload: (() => Promise<void>) & ((route: RouteLocationNormalizedLoaded) => Promise<void>)
}
