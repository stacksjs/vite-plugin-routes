import type { DataLoaderContextBase, DataLoaderEntryBase, DefineDataLoaderOptionsBase_LaxData, DefineLoaderFn, UseDataLoader, UseDataLoaderResult } from 'vite-plugin-routes/data-loaders'
import type { RouteLocationNormalizedLoaded, RouteMap, Router } from 'vue-router'
...
export function defineBasicLoader<Data>(
  ...
  const useDataLoader: UseDataLoaderBasic_LaxData<Data> = () => {
    ...
    if (parentEntry) {
      if (parentEntry === entry) {
        console.warn(
          `👶❌ "${options.key}" has itself as parent. This shouldn't be happening. Please report a bug with a reproduction to https://github.com/stacksjs/vite-plugin-routes`,
        )
      }
